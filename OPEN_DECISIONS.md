...
| How is orientation determined during scanning — auto-detected from crop aspect ratio, or manually set by the operator? | TBD |
| Can the operator override orientation for an already-uploaded photo (e.g., from the Cards Board operator controls)? | TBD |
| Layout algorithm for mixed orientations — how are rows packed when vertical and horizontal cards are mixed? (fixed row-height with variable widths, bin-packing, or another approach) | TBD |
